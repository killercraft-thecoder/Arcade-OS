...
inline Action mkAction(int totallen, RefAction *act) {
...
inline RefRecord *mkClassInstance(VTable *vtable) {
...
inline TValue RefRecord::ld(int idx) {
    // intcheck((reflen == 255 ? 0 : reflen) <= idx && idx < len, PANIC_OUT_OF_BOUNDS, 1);
    return fields[idx];
}

inline TValue RefRecord::ldref(int idx) {
...
inline void RefObject::printVT() {
    ((RefObjectMethod)getVTable(this)->methods[1])(this);
}

inline void RefRecord_destroy(RefRecord *) {}

inline void RefRecord_print(RefRecord *r) {
    DMESG("RefRecord %p size=%d bytes", r, getVTable(r)->numbytes);
}

inline void Segment::set(unsigned i, TValue value) {
    if (i < size) {
        data[i] = value;
...
inline TValue Segment::remove(unsigned i) {
#ifdef DEBUG_BUILD
...
namespace Array_ {
//%
inline bool isArray(TValue arr) {
