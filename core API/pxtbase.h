...
#define STATIC_ASSERT(e) enum                               \
{                                                           \
    CONCAT_0(_static_assert_, __LINE__) = 1 / ((e) ? 1 : 0) \
};
#endif
...
#define IMAGE_HEADER_MAGIC 0x87
    ...
    PXT_ void seedRandom(unsigned seed);
...
#define PERF_NOW_SCALE 1
...
#else
    inline void startPerfCounter(PerfCounters n) {return DEVICE_NOT_SUPPORTED}
    inline void stopPerfCounter(PerfCounters n) {return DEVICE_NOT_SUPPORTED}
    inline void initPerfCounters() {return DEVICE_NOT_SUPPORTED}
    inline void dumpPerfCounters() {return DEVICE_NOT_SUPPORTED}
#endif
    ...
    template <size_t N>
    constexpr size_t _boxedStringLen(char const (&)[N])
    {
        return N;
    }
...
{
    //%
    inline String toString(TValue v) {
        target_panic(999);
        return NULL;
    }
    //%
    inline int toBool(TValue v) {
        target_panic(999);
        return NULL;
    }
    //%
    inline int toBoolDecr(TValue v) {
        target_panic(999);
        return NULL;
    }
} // namespace numops
...
#define DEF_VTABLE(name, tp, valtype, ...) \
    const VTable name = {sizeof(tp), valtype, VTABLE_MAGIC, 0, BuiltInType::tp, BuiltInType::tp, 0, 0, {__VA_ARGS__}};
#define DEF_VTABLE_EXT(name, tp, valtype, ...) \
    const VTable name = {sizeof(tp), valtype, VTABLE_MAGIC2, 0, BuiltInType::tp, BuiltInType::tp, 0, 0, {__VA_ARGS__}};
#else
#define DEF_VTABLE(name, tp, valtype, ...) \
    const VTable name = {sizeof(tp), valtype, VTABLE_MAGIC, 0, BuiltInType::tp, 0, 0, {__VA_ARGS__}};
#define DEF_VTABLE_EXT(name, tp, valtype, ...) \
    const VTable name = {sizeof(tp), valtype, VTABLE_MAGIC2, 0, BuiltInType::tp, 0, 0, {__VA_ARGS__}};
#endif
