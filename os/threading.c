...
typedef struct
{
    ...
    bool isBlocked;     // Whether the thread is blocked
...
bool createThread(void (*task)(void), uint8_t priority, bool isPython)
{
    if (task == NULL)
    ...
    threadPool[threadCount].pythonThread = isPython;
    threadCount++;
    return true;
}
...
void __blockPythonThread(int8_t key)
{
    if (key != 162)
    {
        return;
    ...
    for (uint8_t i = 0; i < threadCount; i++)
    {
        if (threadPool[i].pythonThread)
        {
            threadPool[i].isBlocked = true;
        }
    }
}